...
export default {
  ...
  methods: {
    ...
    acceptOrder(order_id){
        this.requestForm.type = 'accept';
        axios.patch('/api/user/seller/sales/'+order_id, this.requestForm)
        .then(res => {
            toastr['success']("Orderan diterima");
            this.getItems();
        })
        .catch(err => {
            console.error(err); 
    ...
    rejectOrder(order_id){
        this.requestForm.type = 'reject';
        axios.patch('/api/user/seller/sales/'+order_id, this.requestForm)
        .then(res => {
            toastr['error']("Orderan ditolak");
        })
        .catch(err => {
            console.error(err); 
        })
